...
- 오류가 나거나 아무것도 뜨지 않거나 WSL이 설치되지 않았다는 안내가 표시되면 WSL을 설치하고 **Ubuntu**까지 함께 설치합니다.
...
Ubuntu 배포판 설치가 끝나면 자동으로 Ubuntu가 처음 실행되면서 **기본 Unix 사용자 계정과 비밀번호를 먼저 생성하라는 메시지**가 나타납니다.

- 이때 **사용자 이름**과 **비밀번호**를 직접 입력해 계정을 만들고,
- 이 계정이 이후 Ubuntu(WSL)에서 기본 로그인 계정이 됩니다.

이 계정 생성 과정을 마친 뒤에야 정상적으로 Ubuntu 터미널을 사용할 수 있습니다.

설치가 끝나면, 자동으로 WSL 프롬프트로 진입하면 성공입니다.
